...
<img src="public/Logo/icon.png" alt="OpenGal" width="128" />

# OpenGal

**一个专门面向数字生命模拟的集合了语音功能，长期记忆，自主决策的一体化Harness**
...
| L2 结构化事实 | 叙事事实（按主体分类）+ 证据计数 + 时间有效性 + 手动豁免 | SQLite `facts` / `stories` |
...
- [x] 存档式上下文压缩（1M 窗口）
- [x] 导演三态决策 + TTS 打断
- [x] 写入流水线：事实提取 + 冲突仲裁 + 时间有效性 + 遗忘曲线
- [x] 检索注入 + 冻结快照
- [x] 本地向量检索（ONNX embedding，模型不可用时回退关键词匹配）
